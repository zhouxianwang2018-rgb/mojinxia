...
| 8 | 质量 | 引擎二排名无区分度 — score全空、行业全"未知"、理由全"Top+X%" | `engine2_ranker.py` get_sector() 永远返回"未知"，优先级 = 日涨跌幅%而非引擎打分（2026-06-05 发现） |
| 9 | 质量 | 双引擎交叉命中结构性为零 — 引擎一量价异动 vs 引擎二均线趋势天然不重叠 | 引擎一 candidate_pool 和引擎二 ranked 完全没有交集，交叉验证形同虚设（2026-06-05 发现） |
| 10 | 质量 | 收盘复盘无质量门 — 引擎二输出全部 score=None 仍照常写 scenario | 太极实业(600667) score=None, industry=? 排第一，直接进入 momentum_build scenario 并被午盘买入（2026-06-05 案例） |
...
**🆕 引擎二质量修复（2026-06-05 补丁）**：

`engine2_ranker.py` 不再"不改"——以下三个缺陷必须在合并 CRON 前修复：

| # | 缺陷 | 修复 | 位置 |
|:--|------|------|:--|
| ① | `get_sector()` 用 `mx_query("申万行业")` 永远返回"未知" | 改用板块 K 线表字段码（`329915`/`330018`/`326808`）查板块涨跌幅，判断所属主线板块 | `engine2_ranker.py:38‑50` |
| ② | 优先级完全取决于日涨跌幅（+10%→+2, +5%→+1），标签误导"引擎二Top+X%" | 加列表位置分（前3 +2，前5 +1），标签改正为 `引擎二#N·日涨+X%` | `engine2_ranker.py:85‑97` |
| ③ | 交叉命中为空 + 最高 priority≤2 → 收盘复盘无感知，照常写 scenario | 收盘复盘加质量门：交叉命中为空 且 最高 priority≤2 → 不写 momentum_build scenario，改为 `observe_only`（引擎候选池质量不足，今日只观察不交易） | 收盘复盘 prompt Step 4 |
| ④ | 行业数据缺失导致主线匹配永远不触发（主线和行业全是"未知"的比较） | 行业统一从板块扫描 K 线表提取，与 Step 3.5 的主线判断用同一数据源 | `engine2_ranker.py:get_sector()` |
...
| 情况 | 收盘复盘行为 | scenario 类型 |
|:--|:--|:--|
| 引擎一 + 引擎二都正常输出，交叉命中≥1 | 正常生成 momentum_build | `momentum_build` |
| 引擎有输出但交叉命中为空 + 最高 priority≥3 | 降级生成，只取 Top3 但标注"无交叉验证" | `momentum_build` + deviation 标注 |
| 引擎有输出但交叉命中为空 + 最高 priority≤2 | **不写 momentum_build**，改为 `observe_only` | `observe_only`（今日只观察，不交易） |
| 引擎一或引擎二完全无输出 | 跳过候选标的节，不生成新开仓 scenario | — |

> **设计意图**：以前引擎输出不管多烂都会被写成 scenario → 午盘/尾盘照常买入。现在加质量门——垃圾输出导致 `observe_only`，当天不交易反而安全。审计时 `observe_only` 天数会成为引擎质量的可见指标。
...
| 节点 | 内容 | 预计 |
|:--|:--|:--|
| **Phase 0** | **引擎二质量补丁**（修复 ①~④）：get_sector 改用板块K线字段、加列表位置分、标签改正、收盘复盘加质量门 | **今天**（与 SRS-013 架构重构独立，立即生效） |
| Phase 1 | 创建引擎扫描 CRON + 移除旧双 CRON + 验证报纸图 | 本周六 |
| Phase 2 | 冷启动 engine1_pool.json + 创建池管理 CRON | 本周六 |
| Phase 3 | 创建 engine_tracker.json + 尾盘匹配逻辑 + 消除独立选股 + 质量门写入收盘复盘 prompt | 本周六 |
...
| 2026-06-05 | 补充：太极实业案例触发的三个质量缺陷（#8/#9/#10）+ 引擎二质量修复设计（①~④）+ 质量门降级矩阵 + Phase 0 前置补丁 |
