...
| 12 | 收盘复盘凭感觉选股（无引擎数据驱动） | P1 | ✅ 三体联动（6/2上线） |
...
| 层级 | 频率 | 产出 | 规则 |
|------|------|------|------|
| 盯盘 | 每日 15:05 | issues 记录 | 盘中 09:00-15:05 不修改任何 CRON |
| 审计+排期 | 每周六 | 计分卡 + Scenario 审计 + 集中修复 | 改完跑验证 |
| 大版本 | 每月末 | 月复盘 + roadmap 刷新 + 版本号 | 改 3+ CRON → 版本号 +0.1 |

产品设计文档见 `references/design-*`（changelog、roadmap、ADR）。

### 变更安全

- 盘中不改 · 改完必跑验证 · 改 CRON prompt 同步更新本 skill
- 回滚用 `[REVERTED]` 标注，不删除
