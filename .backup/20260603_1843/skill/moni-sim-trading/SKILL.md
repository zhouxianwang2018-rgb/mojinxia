...
## v1.x 引擎一动态候选池（SRS 设计中）

> 完整设计文档：`products/摸金虾/roadmap/需求池/SRS-002-dynamic-pool.md`  
> 状态：🟡 方案讨论中 — 文件布局+数据结构已定稿，流入逻辑+淘汰规则待讨论

**核心改动**：引擎一观察池从固定 55 只改为全市场动态池——每日 mx 选股器自动流入异动标的，20 日无进展自然淘汰，周度概念主线补充。池文件统一路径 `~/.hermes/cron/state/moni/engine1_pool.json`，单脚本 `engine1_pool_manager.py` 管理流入/淘汰/周度补充。
